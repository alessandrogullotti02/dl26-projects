...
def train(config, device, output_dir, seed):
    """Una replica Vanilla DQN; il protocollo primario usa i seed 42,2,3 separatamente."""
    p = config["parameters"]
    ...
    ep_reward = 0.0
    rewards_history, step_history, loss_history = [], [], []
    checkpoint_results = {}
    policy_net.train()
    ...
    for step in range(1, p["TOTAL_STEPS"] + 1):
        ...
        if step > p["LEARNING_STARTS"] and step % p["TRAIN_FREQ"] == 0:
            loss_value, _ = optimize(
                policy_net, target_net, optimizer, loss_fn, buffer,
                p["BATCH_SIZE"], p["GAMMA"], double=False,
            )
        ...
        if step in p["CHECKPOINTS"]:
            ...
            save_checkpoint(
                ...
                f"dqn_pong_seed{seed}_{step}.pth",
