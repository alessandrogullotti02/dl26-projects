...
def train(config, device, output_dir, label, contact_bonus, seed):
    ...
    for step in range(1, config["parameters"]["TOTAL_STEPS"] + 1):
        ...
        if terminated or truncated:
            ...
            if len(history["raw_return"]) % 20 == 0:
                ...
                print(
                    f"{label} | step {step:7d}/{config['parameters']['TOTAL_STEPS']} | ep {len(history['raw_return']):4d} | raw20 {np.mean(history['raw_return'][-20:]):6.2f} | shaped20 {np.mean(history['shaped_return'][-20:]):6.2f} | contatti20 {np.mean(history['contact_count'][-20:]):5.1f} | rally20 {np.mean(history['mean_rally'][-20:]):6.1f} | eps {epsilon:.3f} | {elapsed:.1f}m",
                    flush=True,
                )
            ...
            save_checkpoint(
                {
                    ...
                    "detector_kwargs": config["parameters"]["CONTACT_DETECTOR_KWARGS"],
